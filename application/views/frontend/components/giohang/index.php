<div class="row content-cart">
								...
								<tbody>
									<?php foreach ($cart as $key => $value) :
										...
										<tr>
											...
											<td>
												<a class="remove" id="remove" title="Xóa" onclick="onRemoveProduct(<?php echo $row['id']; ?>)"><i class="fas fa-trash-alt"></i></a>
											</td>
										</tr>
									<?php endforeach; ?>
								</tbody>
							</table>
							<button class="btn btn-primary" onclick="window.location.href='san-pham'"> <a style="color: #FFFFFF" href="<?php echo base_url() ?>san-pham">Tiếp tục mua hàng</a></button>
							<button class="btn btn-success" id="update">Cập Nhật Giỏ Hàng</a></button>
						</div>
						<?php $total = 0; ?>
						<?php foreach ($cart as $key => $value) : 
							$row = $this->Mproduct->product_detail_id($key);?>
	...
	<script>
		...
		$('#remove').click(function(){
					document.location.reload(true);
					alert('Xoá sản phẩm giỏ hàng thành công !');
				});
	</script>